...
_PROMPT = """사용자가 다음 {classification}을 접수했습니다.

[접수 내용]
{message}

이 {classification}을 처리 가능한 수준의 공식 제안서로 만들기 위해
핵심 정보가 부족합니다. 아래 기준으로 질문 {n}개를 생성하세요.

질문 작성 기준:
- 구체적인 장소, 날짜, 피해 규모 등 사실관계 확인
- 이미 시도한 해결책이 있는지 (행정 절차 중복 방지)
- 영향받는 사람 수나 범위 파악
- 원하는 해결 방향이 무엇인지
- 관련 사진, 증빙 자료 보유 여부

각 질문은 30자 이내의 명확하고 구체적인 한 문장으로 작성하세요."""
